...
CREATE TABLE housing_export AS 
	...
	OR (earliest_effectivedate IS NULL AND status_q::date >= '2010-01-01' AND status_q::date <=  '2018-06-30')
	OR (earliest_effectivedate IS NULL AND status_q IS NULL AND status_a::date >= '2010-01-01' AND status_a::date <=  '2018-06-30'))
	AND (dcp_occ_category = 'Residential' OR occ_prop LIKE '%Residential%' OR occ_init LIKE '%Residential%' OR occ_prop LIKE '%Assisted%Living%' OR occ_init LIKE '%Assisted%Living%')
	AND (occ_init <> 'Garage/Miscellaneous' AND occ_prop <> 'Garage/Miscellaneous')
	AND job_number NOT IN (
		SELECT DISTINCT job_number 
		FROM housing
		WHERE dob_type = 'NB' AND occ_prop = 'Hotel or Dormitory' AND x_mixeduse IS NULL)
	);
-- export
--all records
\copy (SELECT * FROM dev_export) TO '/prod/db-housing/housing_build/output/devdb_developments.csv' DELIMITER ',' CSV HEADER;
-- only points
\copy (SELECT * FROM dev_export WHERE ST_GeometryType(geom)='ST_Point') TO '/prod/db-housing/housing_build/output/devdb_developments_pts' DELIMITER ',' CSV HEADER;
-- records that did not geocode
\copy (SELECT * FROM dev_export WHERE geom IS NULL AND latitude IS NULL AND upper(address) NOT LIKE '% TEST %') TO '/prod/db-housing/housing_build/output/devdb_developments_nogeom' DELIMITER ',' CSV HEADER;
-- only housing records
\copy (SELECT * FROM housing_export) TO '/prod/db-housing/housing_build/output/devdb_housing.csv' DELIMITER ',' CSV HEADER;
-- only housing points
